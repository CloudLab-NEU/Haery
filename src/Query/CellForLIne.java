...
public class CellForLIne {
	...
	public List<String> findCellByDimensionValue(List<String> queryDimension,
			...
			List<String> targetKey) throws NotPositiveException {
		...
		/*	if (targetIndex.get(i).isEmpty()) {
				...
				if (segs.size() > 1) {
					filtterData.add(segs.get(
							segs.size() - 1));
					if (segs.size()>2) {
						notfiltterData.addAll(segs.subList(2, segs.size()-1));
					}
				}
				
				if (notfiltterData.size()!=0) {
					targetIndexWillNotBeFiltter.add(i, notfiltterData);
				}
				
				targetIndexWillBeFiltter.add(i, filtterData);
			}
			
		}
		hkhjk
		return linerazeAllTheCell(targetIndex, targetKey);*/
	...
	private List<String> backtrace(List<Segment> left, List<String> seqnum)
		...
		for (int i = 0; i < seqnum.size(); i++) {
			...
			if (seq != -1) {
				file = new FlatZordering(left.size()).encode(left, ratios)
						+ "sd";
				resList.add(file);
			} else {
				file = new FlatZordering(left.size()).encode(left, ratios)
		...
		while (true) {
			...
			for (int i = 0; i < lastesVersionSegments.size(); i++) {
				...
				if (latestVersion == minimalSegmentVersionOnDimension
					...
					left.set(index, dimension.backtraceOnSegment(segment));
				}
			}
			for (int i = 0; i < seqnum.size(); i++) {
				int seq = Integer.parseInt(seqnum.get(i));
	...
	public static void main1(String[] args) {
		CellForLIne cell = new CellForLIne();
	}
